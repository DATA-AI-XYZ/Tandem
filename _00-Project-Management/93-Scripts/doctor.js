...
// Kit version shipped with the scripts (manifest is the source-of-truth that travels with
// the tooling). Used for the read-only update-available drift notice (STORY-16.4.03).
const KIT_MANIFEST = (() => { try { return require('./lib/pm-manifest.json'); } catch (_e) { return null; } })();
const KIT_VERSION = (KIT_MANIFEST && KIT_MANIFEST.kitVersion) || null;
...
function diagnose(REPO_ROOT) {
  ...
  // Distinguish a MISSING package.json from a present-but-MALFORMED one: readJson
  // returns null for both, but the operator's fix differs (create vs fix-the-JSON),
  // so the wording must name the file and the corrective action (BACKLOG-0056).
  const pkgPath = path.join(REPO_ROOT, 'package.json');
  const pkgExists = fs.existsSync(pkgPath);
  const pkg = pkgExists ? readJson(pkgPath) : null;
  const scripts = (pkg && pkg.scripts) || {};
  if (!pkgExists) bad('package.json not found', `${pkgPath} — pm:* scripts cannot be wired (run pm:install)`);
  else if (!pkg) bad('package.json malformed', `${pkgPath} is present but not valid JSON — fix the JSON syntax`);
  else {
  ...
  // Resolve the standards folder via the path map (not the literal '90-Standards') so a
  // custom layout that relocates it is still probed at the right place (BACKLOG-0056).
  // loadPaths is tolerant (never throws on a missing/malformed config), so resolving here
  // — before the validity check — is safe; map/layout/source are reused by step 4 below.
  const { map, layout, source } = loadPaths(PM_ROOT);
  const pmPaths = path.join(PM_ROOT, map.standards, 'pm-paths.json');
  ...
  if (!pmPathsExists && !cfgExists) {
    ...
    if (!valid) bad('Path map invalid', 'present but not valid JSON — fix it');
    else ok('Path map present + valid', which.join(' + '));
  }

  // 4. layout resolves + which mapped folders exist (report; not core)
  // (map/layout/source already resolved in step 3 above so the standards folder could
  //  be path-mapped; reuse rather than re-call loadPaths.)
  ...
  // 5. kit-version drift (READ-ONLY notice — never writes; the fix is the manual pm:update).
  // Compares the installed `kitVersion` (.claude-pm-config.json, stamped by install/update)
  // against the version shipped with these scripts. A mismatch (or absent stamp) prints an
  // "update available" notice; doctor itself touches nothing (ADR-0073: manual update only).
  if (KIT_VERSION) {
    const installed = (cfgExists ? (readJson(cfgPath) || {}) : {}).kitVersion;
    if (!installed) warn('Kit version not stamped', `kit ${KIT_VERSION} available — run \`pm:update\` to refresh + record it`);
    else if (installed !== KIT_VERSION) warn('Update available', `installed kit ${installed} → ${KIT_VERSION}; run \`pm:update\` (non-destructive)`);
