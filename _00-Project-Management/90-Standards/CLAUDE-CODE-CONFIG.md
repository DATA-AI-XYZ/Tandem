...
**Decision rule:** files clustered under one path → **subdir-launch** (default for single-area stories). Files scattered, a cross-cutting refactor, a dependency bump, or codebase-map-spanning work → **root-launch** (the exception list in DAILY-WORKFLOW §5). The role of `PROJECT-CONTEXT.md` in each model (canonical table vs index) is §2.1.3; the project-type gate is §2.1.2 + ADR-0009 — a central PROJECT-CONTEXT or distributed per-directory CLAUDE.md files, chosen by project type. Table placement (why this is §2.1.5, not §2.1.3 as the originating story's AC named): ADR-0019 — the root-vs-subdirectory launch trade-off table lives at CLAUDE-CODE-CONFIG §2.1.5.
...
**Known assumption — the hooks require a bare `node` on PATH.** All three commands in `hooks/hooks.json` are spelled `node "${CLAUDE_PLUGIN_ROOT}/…/hook.js" <event>`, so they resolve `node` from the shell's PATH rather than from an absolute interpreter path. That is not universally true: on at least one maintainer machine (Windows-first development) `node` is absent from PATH and has to be invoked as `"C:\Program Files\nodejs\node.exe"`, which is exactly the environment note carried in `PROJECT-CONTEXT.md`. Where the assumption does not hold, the hooks simply do not run.

This is **documented rather than mitigated, deliberately.** The hooks are already fail-open by design — `hook.js` returns 0 when the kit scripts are absent, and a hook that cannot start is a hook that does nothing rather than a hook that breaks the session. That property is worth more than PATH independence: hard-coding an interpreter path would be wrong on every machine but the one it was written on, and a launcher shim adds a moving part to the one component whose job is to be invisible. So the assumption is stated here, and the consequence of it failing — the dashboard is not auto-regenerated on Stop, and `pm:lint` does not auto-run on edit — is a convenience loss the operator recovers by running `npm run pm:dash` / `npm run pm:lint` directly. Revisit if a consumer reports the hooks silently not firing and PATH turns out to be the common cause.
...
The four hot prompts (06, 07, 08, 09) are also published as model-invoked skills inside the `Tandem` plugin. Plugin skills are **namespaced** (`/tandem:execute-story` in source; `/tandem:execute-story` once the release rewrite has run — see §2.3.0) and **selected by description match** — Claude loads them when the user's task description matches the skill's `description` frontmatter, or when the user invokes them explicitly with the namespaced form.
...
#### 2.3.0 One slash-prefix in source: `/tandem:*`

A skill that names a sibling command has to spell the prefix, and there are two spellings in play: the **dev identity** this repo is called, and the **prod identity** the published plugin is called. Writing both into source is how a reader ends up meeting two contradictory spellings of the same command.

**The rule — source skills write one prefix, the repo-standard `/tandem:*`.** Every `SKILL.md` under `skills/` names sibling commands with that prefix and no other. Checked in one line:

```bash
grep -rEn '/[Tt]andem:' skills/     # in SOURCE: no matches. In a BUILT artifact: every skill matches, by design.
```

**The release rewrite owns the prod identity.** The published `/tandem:*` spelling is produced only by `scripts/release-tandem.js`, whose `REWRITES` table maps the dev name token to the public one and then lowercases the machine identity, because claude.ai's marketplace sync requires kebab-case (ADR-0086 — the public machine identity is kebab-case — plugin `tandem`, marketplace `data-ai-xyz`, skill prefixes `/tandem:*` — stamped by this build, never hand-written into source). A hand-written prod prefix in source is therefore not "already correct": it is a spelling the build would have produced anyway, parked where the build cannot verify it, and it leaves the one-line check above unable to tell a converged file from a stale one.

Two consequences worth stating, because both have bitten:

- **`produced_by:` in a wiki anchor block is a command, so the same rule applies.** The anchor resolver strips the prefix before resolving (`skills/<name>/SKILL.md` is the resolution), so a document stamped under either spelling resolves to the same skill — already-stamped documents are not stranded by a convergence.
- **Non-skill surfaces are not covered by that grep but still ship.** `90-Standards/`, `91-Templates/` and the ship-filtered `93-Scripts/` pass through the same rewrite, so they follow the same convention. The generator's own `WIKI_PRODUCER_COMMAND` constant is a deliberate exception: it is an instruction the *board* prints to a consumer, and a consumer runs the published spelling.

**Reading this inside an installed plugin?** Then this page has already been through that rewrite, and every prefix on it — the heading included — is the published spelling. The rule still describes how the kit's own source is authored.

#### 2.3.1 When to path-scope vs description-match a skill

The blog says *"Scope to paths — bind skills to specific directories so they activate only for relevant work."* As of **2026-05-23** the spec supports this via the **`paths`** frontmatter field on `SKILL.md` — glob patterns (comma-separated string or YAML list) that restrict *model-invocation* to sessions touching matching files. Investigation and exact syntax: ADR-0010 — path-scoped skills are spec-supported through `paths` frontmatter; the kit ships documentation only, with no skill scoped today. The `paths` glob format is the same as `.claude/rules/` path-specific rules (repo-relative globs, `**` recursion, brace expansion like `src/**/*.{ts,tsx}`).
...
The kit now ships one genuinely path-scoped skill, **`skills/path-scope-example/`** — introduced by [STORY-17.2.01](../32-Stories/EPIC-17/FEAT-17.2/STORY-17.2.01-apply-paths-scope-candidate.md) (ADR-0076 — path-scoping is proven by a minimal reference candidate skill, because no existing skill qualifies) precisely to prove the mechanism on real code. It carries `paths: ["_00-Project-Management/40-Decisions/**/*"]`, so it auto-loads **only** while Claude is editing an ADR under the decisions folder and stays silent everywhere else — a genuinely directory-local concern (ADR numbering/format conventions). Its own "When this activates" section documents both branches (in-scope → activates, out-of-scope → silent); it used to carry a separate companion file saying the same thing, removed by STORY-35.3.04 because a second file that only restated the skill's own body was one more thing to keep in sync. Copy that skill's `paths:` block as the template for your own directory-scoped skills. (It is the *only* path-scoped skill in the kit; every other Tandem skill — `core`, `execute-story`, `close-out-story`, the rest — stays description-matched, for the reasons above.)
