...
- In a **consumer** repo, the live `DASHBOARD.html` is generated — never `git add` it by hand; `pm:install` gitignores it automatically (ADR-0082) so `git add` / phase-close merges can't fail on it. Fix if you need to see it: `npm run pm:dash` rebuilds it for local viewing.
