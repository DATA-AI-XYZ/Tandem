...
**`.claude/settings.json` baseline** ships at the kit's `scaffold/.claude/settings.json` and drops to `.claude/settings.json` (repo root) on install, with a destructive-only `permissions.deny` floor (`rm -rf`, `git push --force`, `npm publish`, `.env` writes, `node_modules` writes). Rationale + the trim/expand path live in the file's `_comment` header. Per the [Claude Code settings docs](https://code.claude.com/docs/en/settings), **permissions merge across scopes** (user + project + local) — project deny rules *add to*, never weaken, an operator's own list — and `deny` is evaluated before `ask` before `allow`, first match wins. See also the [large-codebases blog](https://claude.com/blog/how-claude-code-works-in-large-codebases-best-practices-and-where-to-start): "commit `permissions.deny` rules in `.claude/settings.json`." The file is strict JSON (Anthropic docs don't document comment tolerance — the header lives in a `_comment` field).
...
Now the skill auto-loads only when Claude is working with files under `solutions/`, and stays silent during front-end work — same repo, different directory, correct scoping. (No skill in *this* kit qualifies today; the kit ships documentation-only and adopts `paths` for the first directory-specific skill that arrives — see ADR-0010.)
